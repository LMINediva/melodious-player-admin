    ...
    <el-table :data="tableData" stripe style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="title" label="公告标题" width="120" align="center">
        <template v-slot="scope">
          <p class="cell-overflow-ellipsis">{{ scope.row.title }}</p>
        </template>
      </el-table-column>
      <el-table-column prop="content" label="公告内容" width="200" align="center">
        <template v-slot="scope">
          <p class="cell-overflow-ellipsis">{{ scope.row.content }}</p>
        </template>
      </el-table-column>
      <el-table-column prop="publishTime" label="发布时间" width="200" align="center" :formatter="formatDateTime"/>
...
const handleDelete = async (id) => {
  ...
  const res = await requestUtil.post("sys/notice/delete", ids);
